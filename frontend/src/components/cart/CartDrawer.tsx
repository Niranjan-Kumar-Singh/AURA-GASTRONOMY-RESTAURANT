        ...
        >
          ...
          <div className="p-4 border-b border-aura-border flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="font-serif font-bold text-lg text-aura-ivory">Your Dining Cart</h2>
                <p className="text-[10px] text-aura-gold/90 font-semibold tracking-wide">Gourmet Selection &amp; Add-ons</p>
              </div>
            </div>
          ...
          <div className="p-4 flex-1 overflow-y-auto space-y-4">
            ...
            {items.length > 0 && (() => {
              ...
              return (
                <div className="p-3 bg-gradient-to-r from-amber-500/15 via-aura-obsidian to-aura-gold/15 border border-aura-gold/50 rounded-2xl space-y-2 shadow-lg">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center space-x-1.5 font-bold text-aura-gold">
                      <Gift className="w-4 h-4 text-aura-gold animate-bounce" />
                      <span>{spendTier.isUnlocked ? '🎉 Reward Unlocked!' : 'Spend & Unlock Rewards'}</span>
                    </div>
                    <span className="text-[10px] font-mono text-aura-ivory font-bold bg-aura-gold/20 px-2 py-0.5 rounded-full border border-aura-gold/30">
                  ...
                  <div className="w-full bg-aura-obsidian h-2.5 rounded-full overflow-hidden border border-aura-border p-0.5">
                    <div
                      className="bg-gradient-to-r from-amber-400 to-aura-gold h-full rounded-full transition-all duration-500 shadow-[0_0_10px_rgba(212,175,55,0.6)]"
                  ...
                  <div className="space-y-2">
                    {spendTier.isUnlocked ? (
                          ...
                          {spendTier.rewardOptions.map((opt) => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  if (hasAnyClaimed) {
                                    showToast('Limit 1 Free Reward per table session!', 'info');
                                    return;
                                  }
                                  ...
                                  addItem(freeRewardItem, 1, `🎁 Free Reward Tier ${spendTier.tierLevel} (₹0)`);
                                  showToast(`Claimed "${opt.name}" for ₹0.00!`, 'success');
                                }}
                                disabled={isClaimed}
                                className={`px-2.5 py-2 font-bold text-[10px] rounded-xl flex items-center justify-center space-x-1 shadow-md transition-transform active:scale-95 cursor-pointer ${
                                  isClaimed
                                    ? 'bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 cursor-not-allowed'
                                    : 'bg-emerald-500 hover:bg-emerald-600 text-aura-obsidian'
                                }`}
                      ...
                      <span className="text-aura-ivory text-[11px]">
                        Add <strong className="text-aura-gold font-mono">₹{spendTier.amountNeeded}</strong> more for <span className="text-amber-300 font-bold">{spendTier.reward}</span>
                      </span>
                    )}
                  </div>
                    ...
                    <div className="pt-1 flex items-center gap-1.5 overflow-x-auto no-scrollbar">
                      ...
                      {spendTier.quickBoosters.map((booster) => (
                        <button
                          ...
                          className="px-2 py-1 bg-aura-gold/20 hover:bg-aura-gold text-aura-gold hover:text-aura-obsidian border border-aura-gold/40 rounded-lg text-[10px] font-bold shrink-0 transition-all flex items-center space-x-1 active:scale-95 cursor-pointer"
                        >
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-0.5 font-mono text-xs font-bold">
                        {item.menuItem.price === 0 ? (
                          ...
                          <span className="text-aura-gold">₹{item.menuItem.price * item.quantity}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 bg-aura-container border border-aura-border px-2.5 py-1 rounded-xl w-fit">
                        ...
                        <button
                          ...
                          className={`font-bold ${item.menuItem.price === 0 ? 'text-aura-slate/40 cursor-not-allowed' : 'text-aura-ivory hover:text-aura-gold'}`}
                        >
            ...
            {items.length > 0 && (
              <div className="p-3.5 bg-gradient-to-r from-aura-gold/10 via-aura-obsidian to-aura-gold/10 border border-aura-gold/40 rounded-2xl space-y-2.5 shadow-xl">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-1.5 text-xs font-bold text-aura-gold uppercase tracking-wider">
                    <Sparkles className="w-4 h-4 text-aura-gold animate-pulse" />
                ...
                <div className="overflow-x-auto no-scrollbar flex items-center space-x-3 py-1">
                  {AI_RECOMMENDED_PAIRINGS.map((pairing) => {
                    ...
                    return (
                      <div
                        key={pairing.id}
                        className="w-44 bg-aura-obsidian border border-aura-border/70 hover:border-aura-gold/60 rounded-xl p-2.5 shrink-0 space-y-2 shadow-md flex flex-col justify-between"
                      >
                        <div className="flex items-start space-x-2">
                          ...
                          <div className="min-w-0">
                            <h5 className="font-serif font-bold text-aura-ivory text-xs truncate leading-tight">
                              {pairing.name}
                            </h5>
                            <span className="font-mono text-aura-gold font-bold text-[11px]">₹{pairing.price}</span>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className={`w-full py-1.5 px-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all flex items-center justify-center space-x-1 cursor-pointer ${
                            isAlreadyInCart
                              ...
                              : 'bg-aura-gold hover:bg-aura-gold-hover text-aura-obsidian shadow-md active:scale-95'
                          }`}
                        >
                          <Plus className="w-3 h-3" />
